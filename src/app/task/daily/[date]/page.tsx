        ...
        <div className="bg-white border border-stone-200 rounded-2xl overflow-hidden">
          <div className="border-b border-stone-200 px-5 py-4">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-sm font-semibold text-stone-900">Tasks</h2>
                <p className="text-xs text-stone-400 mt-0.5">{completed}/{total} completed</p>
              </div>
              <div className="flex items-center gap-2 flex-wrap">
                <button
                  onClick={openClearConfirm}
                  disabled={state.plans.length === 0 || state.isResetting}
                  className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-stone-600 border border-stone-200 hover:bg-stone-50 disabled:opacity-40 disabled:cursor-not-allowed rounded-lg transition-colors"
                >
                  ...
                  {state.isResetting ? 'Clearing...' : 'Clear all'}
                </button>
                <button
          ...
          <div className="divide-y divide-stone-200/50">
            {sortedPlans.length === 0 ? (
              <div className="p-8 text-center">
                <div className="p-3 bg-stone-100 rounded-xl inline-block mb-4">
              ...
              periodOrder.map((period) => {
                ...
                return (
                  <div key={period} className="divide-y divide-stone-200/50">
                    <div className="px-5 py-2.5 bg-stone-50 border-b border-stone-200 flex items-center gap-2">
                    ...
                    {list.length === 0 ? (
                      ...
                      list.map((plan, idx) => (
                        <div key={plan.id} className="px-5 py-4 hover:bg-stone-50 transition-colors">
                          <div className="flex items-start gap-4">
                            {/* Status + time */}
                            <div className="flex items-center gap-2 flex-shrink-0 pt-0.5">
                              {plan.startTime && (
                                <span className="text-xs font-medium text-emerald-600 bg-emerald-50 border border-emerald-200 rounded-md px-2 py-0.5">
                              ...
                              <select
                                ...
                                className="text-xs font-medium border border-stone-200 rounded-lg px-2 py-1 bg-white text-stone-700 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 cursor-pointer"
                              >
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex flex-wrap items-center gap-2 mb-1">
                                <span className={`text-sm font-semibold ${
                                  plan.status === 'Done' || plan.status === 'Missed' || plan.status === 'Failed'
                                    ? 'text-stone-400 line-through' : 'text-stone-900'
                                }`}>
                                  {plan.title}
                                </span>
                                {plan.priority && (
                                  <span className={`text-xs px-1.5 py-0.5 rounded font-medium ${
                                    plan.priority === 'high' ? 'bg-red-50 text-red-600' :
                                    plan.priority === 'low' ? 'bg-green-50 text-green-700' :
                              ...
                              {plan.description && (
                                <p className={`text-xs leading-relaxed ${
                                  plan.status === 'Done' || plan.status === 'Missed' || plan.status === 'Failed'
                                    ? 'text-stone-300 line-through' : 'text-stone-500'
                                }`}>
                                  {plan.description}
                                </p>
                              )}
            ...
            <form onSubmit={addTask} className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full px-4 py-3 bg-stone-100 border border-stone-200 rounded-xl text-stone-900 placeholder-stone-400 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 resize-none"
                  disabled={state.isAdding}
                />
              </div>
              <div>
