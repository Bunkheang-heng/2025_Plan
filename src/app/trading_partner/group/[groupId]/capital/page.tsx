    ...
    <div className="min-h-screen bg-theme-primary p-4 sm:p-6 lg:p-8">
      <div className="max-w-3xl mx-auto space-y-6">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold text-theme-primary">Account Capital</h1>
        ...
        <div className="bg-theme-card border border-theme-secondary rounded-2xl p-5">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-xs text-theme-tertiary">Group</div>
              <div className="text-theme-primary font-semibold">{group?.name || groupId}</div>
            </div>
            <div className="text-right">
              <div className="text-xs text-theme-tertiary">Total Account Capital</div>
        ...
        <div className="bg-theme-card border border-yellow-500/20 rounded-2xl p-5">
          <h2 className="text-lg font-semibold text-theme-primary mb-4">Per Person</h2>

          {partners.length === 0 ? (
            <p className="text-sm text-theme-tertiary">No partners configured yet.</p>
          ) : (
                ...
                <div key={p.uid} className="bg-black/30 border border-theme-secondary rounded-xl p-4">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    ...
                    <div className="w-full sm:w-56">
                      <label className="block text-xs text-theme-tertiary mb-1">Capital ($)</label>
                      <input
                        type="number"
                        step="0.01"
                        ...
                        disabled={role !== 'admin'}
                        className="w-full px-4 py-2 bg-gray-900/60 border border-theme-secondary rounded-lg text-theme-primary focus:outline-none focus:border-yellow-500 disabled:opacity-60"
                        placeholder="0"
                      />
                    </div>
                  </div>
