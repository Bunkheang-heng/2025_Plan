...
export default function TradingChartPageClient({
  routeBase
}: {
  ...
  useEffect(() => {
    ...
    if (isBotAccount && !isBotPnL) {
      router.replace(`/trading/bot_trading_pnl/${accountId}/chart`)
    } else if (!isBotAccount && isBotPnL) {
      router.replace(`/trading/trading_pnl/${accountId}/chart`)
    }
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            {allAccounts.length > 1 && (
              <select
                value={accountId}
                onChange={(e) => router.push(`${routeBase}/${e.target.value}/chart`)}
